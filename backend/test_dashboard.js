...
async function runTests() {
  ...
  // Test 1: Unauthorized
  const res1 = await fetchJSON('/dashboard', 'GET');
  assert(res1.status === 401, 'Unauthorized request returns 401');

  // Test 2: Success
  const res2 = await fetchJSON('/dashboard', 'GET', null, token);
  assert(res2.status === 200 && res2.data.success, 'Dashboard request returns 200');
