...
async function runTests() {
  console.log('--- STARTING RESULTS TESTS ---');
  let passed = 0;
  let failed = 0;
  ...
  // Test 1: Unauthorized
  const res1 = await fetchJSON('/results', 'POST');
  assert(res1.status === 401, 'Unauthorized request returns 401');

  // Test 2: Validation error
  const res2 = await fetchJSON('/results', 'POST', { exam_id: 'invalid-uuid' }, token);
  assert(res2.status === 400 || res2.status === 422, 'Invalid request returns 400/422');

  // We rely on DB constraints for the rest of live tests. 
